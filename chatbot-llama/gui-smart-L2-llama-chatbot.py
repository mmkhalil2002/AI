...
# RUN INSTALLER NOW
# ----------------------------------------------------------
# Dependency installation runs from main() after the GUI appears.
...
import shutil
import subprocess
# requests is imported after its dependency check in main().
import re
...
SMART_JSON_FILE = "smart_knowledge_L2.json"
...
# ----------------------------------------------------------
# TIME-SENSITIVE KNOWLEDGE CONFIGURATION
# ----------------------------------------------------------
# PURPOSE
# -------
# Some knowledge becomes outdated after some time.
#
# Example:
#   - "what is ai" usually stays valid for a long time
#   - "population of egypt" can change over time
#   - "weather in cairo" changes very quickly
#   - "usd to egp rate" changes frequently
#
# So we classify questions and assign expiration periods.
# If an answer is expired, the chatbot should NOT trust it
# and should ask the model again to refresh the knowledge.
# ----------------------------------------------------------
...
TIME_SENSITIVE_KEYWORDS = {
    ...
    "temperature": "weather",
    "forecast": "weather",
    "today": "current_affairs",
    "current": "current_affairs",
    "latest": "current_affairs",
    "now": "current_affairs",
    "president": "politics",
...
# Expiration rules by category.
# None means "does not expire automatically".
#
# Examples:
#   definition      -> stable knowledge
#   demographics    -> may change, but not every hour
#   finance         -> changes quickly
#   weather         -> changes very quickly
#   politics        -> can change with elections/appointments
#
CATEGORY_TTL = {
    "definition": None,
    "general_fact": None,
...
# ----------------------------------------------------------
# TIME-SENSITIVE DETECTION AND EXPIRATION HELPERS
# ----------------------------------------------------------
# PURPOSE
# -------
# These helper functions determine:
#   1) whether a question is time-sensitive
#   2) what category it belongs to
#   3) how long its answer should remain valid
#   4) whether a stored answer is expired
# ----------------------------------------------------------
...
def detect_question_category(question):
    """
    Try to classify the question into a category.

    Example:
        "population of egypt" -> demographics
        "what is ai"          -> definition
        "weather in cairo"    -> weather

    If no special keyword is detected, we fall back
    to a stable category.
    """
    norm_q = normalize_text(question)

    for keyword, category in TIME_SENSITIVE_KEYWORDS.items():
        if keyword in norm_q:
            return category

    # If question starts with "what is", "who is", "define",
    # it often behaves like a stable knowledge question.
    if norm_q.startswith("what is ") or norm_q.startswith("who is "):
        return "definition"
...
def is_time_sensitive_question(question):
    """
    Return True if the detected category has a TTL.
    Return False if the category is considered stable.
    """
    category = detect_question_category(question)
    ttl_days = CATEGORY_TTL.get(category, CATEGORY_TTL["unknown"])
    return ttl_days is not None


def get_expiration_for_question(question):
    """
    Compute expiration datetime for a question based on category.

    Example:
        demographics -> expires in 30 days
        weather      -> expires in 0.25 days (6 hours)
        definition   -> no expiration (None)
    """
    category = detect_question_category(question)
    ttl_days = CATEGORY_TTL.get(category, CATEGORY_TTL["unknown"])
    ...
    return get_now() + timedelta(days=ttl_days)


def is_entry_expired(entry):
    """
    Check whether a stored JSON entry is expired.

    Rules:
    - if entry says it is not time-sensitive -> never expired
    - if expires_at is missing               -> treated as not expired
    - if current time is after expires_at    -> expired
    """
    is_time_sensitive = bool(entry.get("is_time_sensitive", False))
    ...
    try:
        ...
        for item in raw:
            if isinstance(item, dict) and "question" in item and "answer" in item:
                # If older entries do not have metadata yet, fill defaults.
                item.setdefault("is_time_sensitive", is_time_sensitive_question(item["question"]))
                item.setdefault("category", detect_question_category(item["question"]))
                item.setdefault("created_at", None)
                item.setdefault("expires_at", None)
                item.setdefault("source", "json")
                item.setdefault("model_used", None)
...
def save_smart_knowledge(question, answer, source="model", model_used=None):
    print_line()
    ...
    # ----------------------------------------------------------
    # DUPLICATE / REFRESH HANDLING
    # ----------------------------------------------------------
    # If the same normalized question already exists:
    #   - and is expired -> replace / refresh that entry
    #   - and is still valid -> skip save
    # ----------------------------------------------------------
    existing_index = None
    ...
    created_at = get_now()
    expires_at = get_expiration_for_question(question)
    ...
    if existing_index is not None:
        existing_entry = data[existing_index]

        if is_entry_expired(existing_entry):
            print("[SAVE] Existing question found, but entry is expired.")
            print("[SAVE] Replacing expired entry with refreshed answer...")
            data[existing_index] = new_entry
        else:
            print("[SAVE] Duplicate question found and existing entry is still valid. Save skipped.")
            return False
    else:
    ...
    # Rebuild memory from the updated file content to stay consistent
    knowledge_data.clear()
    for item in data:
        knowledge_data.append(item)

    print("[SMART LEARN] Added new knowledge successfully.")
    print(f"[SMART LEARN] Category          : {category}")
    print(f"[SMART LEARN] Time-sensitive    : {time_sensitive}")
    print(f"[SMART LEARN] Created at        : {format_dt(created_at)}")
    print(f"[SMART LEARN] Expires at        : {format_dt(expires_at)}")
...
def search_knowledge(user_text):
    ...
    for item in knowledge_data:
        ...
        if user_norm == q_norm:
            # Time validity check
            if is_entry_expired(item):
                if DEBUG_SEARCH:
                    print("[DEBUG] EXACT NORMALIZED MATCH FOUND BUT ENTRY IS EXPIRED")
                return None
    ...
    for item in knowledge_data:
        ...
        if user_reduced and q_reduced and user_reduced == q_reduced:
            if is_entry_expired(item):
                if DEBUG_SEARCH:
                    print("[DEBUG] EXACT REDUCED MATCH FOUND BUT ENTRY IS EXPIRED")
                return None
    ...
    for item in knowledge_data:
        ...
        if shorter_token_count >= 2:
            if q_reduced in user_reduced or user_reduced in q_reduced:
                if is_entry_expired(item):
                    if DEBUG_SEARCH:
                        print("[DEBUG] STRONG CONTAINS MATCH FOUND BUT ENTRY IS EXPIRED")
                    return None
    ...
    if best_score >= MIN_MATCH_SCORE:
        if best_item is not None and is_entry_expired(best_item):
            if DEBUG_SEARCH:
                print("[DEBUG] TOKEN OVERLAP MATCH FOUND BUT ENTRY IS EXPIRED")
            return None
...
def answer_user_question(user_text):
    ...
    return "No usable answer was returned. Check Ollama and the installed models."
...
def main():
    ...
    def ask():
        ...
        status_label.config(text="Checking knowledge and asking LLaMA...")
        answer_box.delete("1.0", tk.END)
