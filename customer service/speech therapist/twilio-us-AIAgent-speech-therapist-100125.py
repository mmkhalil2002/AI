...
def voice():
    ...
    if stage == "intro":
        ...
        # ----------------------------------------------------------------------
        # 📍 Booking flow:
        # The caller has just been asked to pick a doctor.
        # Accepts either speech OR single-digit DTMF input (doctor_dtmf_map).
        # ----------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        if dtmf_digits:
            doctor_map = session_data[call_sid].get("doctor_dtmf_map", {})
            chosen_name = doctor_map.get(dtmf_digits)
            if chosen_name:
                # Find doctor_id by matching the friendly name
        ...
        if matched_id is None:
            ...
            if not spoken_clean or spoken_clean in junk_inputs or len(spoken_clean) < 3:
                ...
                # 👇 IMPORTANT: allow both speech + dtmf here
                gather = make_gather(
                    "Please say the name of the doctor you'd like to book with, or press the number.",
            ...
            if len(partial_matches) == 1:
                ...
                matched_id = partial_matches[0][0]  # fallback to first
        ...
        if matched_id is None:
            ...
            gather = make_gather(retry_prompt, hints=doctor_list_str, num_digits=1, input="speech dtmf")
            resp.append(gather)
            return str(resp)
        ...
        # 👇 Allow both speech and dtmf for phone entry
        gather = make_gather(phone_prompt, num_digits=10, input="speech dtmf")
        resp.append(gather)
        return str(resp)
    ...
    #   - Every code path returns `str(resp)` (Flask requirement).
        ...
        if not doctor_id:
            debug_print("ask_time_date: ❌ no doctor selected → booking restart")
