...
#SPEECH_INPUT_DURATION  = 15
#MAX_RECORD_TIME = 60
#MAX_NUMBER_DR_RETRY = 3
#MAX_APPT_RETRIEVED_FROM_CALNDER = 50
# Voice-bot parameters
...
def gpt_speak(prompt):
    """
    Generate a dynamic response using ChatGPT (GPT-4) based on a given prompt.
    
    Parameters:
        prompt (str): The user message or query that needs a natural language response.
    
    Returns:
        str: The response text generated by the ChatGPT model.
    """

    # Call the OpenAI ChatCompletion endpoint to get a reply from GPT-4.
    response = openai.ChatCompletion.create(
        model="gpt-4",  # Specify the model (e.g., GPT-4)
        messages=[
            # Set the assistant’s behavior/personality using the system role.
            {"role": "system", "content": "You are a friendly AI assistant for a therapist clinic."},

            # The user's message to which the assistant should reply.
            {"role": "user", "content": prompt}
        ]
    )

    # Extract and return the assistant's reply text from the response object.
...
def cancel_event_by_phone(
        calendar_id: str,
        phone: str,
        spoken_day: str | None = None,     # e.g. "Monday" or "July 14"
        spoken_time: str | None = None,    # e.g. "2 PM" or "14 00"
        creds=None
    ) -> bool:
    """
...
def voice():
    ...
    if stage == "intro":
      ...
      # Save the current session state as moving to the next stage ("intent")
       ...
       prompt = "Greet the customer cheerfully and ask if they want to book an appointment or leave a message."
       ...
       # Check if the word "message" appears in the user's spoken input (case-insensitive)
        ...
        for doc_id, friendly in googleid_dr_name_map.items():
            if friendly.lower() in spoken_text:
        ...
        if matched_id is None:
            ...
            if extracted:
                for doc_id, friendly in googleid_dr_name_map.items():
                    if friendly.lower() == extracted.lower():
                        matched_id = doc_id
                        break
