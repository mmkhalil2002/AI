# update  09/03/25 time_saved 07:24 am
# =========================
...
def is_time_slot_available(calendar_id: str, start_time: str, end_time: str, creds) -> bool:
    ...
    Validations (done BEFORE Google):
      - Start/end parseable (UTC tolerated; naive assumed UTC).
      - Start < End; same local date (no cross-midnight sessions).
      - **Start is not in the past relative to 'now' in clinic local time.**
    ...
    _dbg(f"is_time_slot_available: 🔎 cal='{calendar_id}' window={start_time}→{end_time}")

    # ---- parse inputs (treat naive as UTC) ----
    try:
    ...
    # helper to coerce lunch envs to time
    def _as_time(val, default_h=12, default_m=0) -> dtime:
        if isinstance(val, dtime): return val
        if isinstance(val, int):   return dtime(max(0, min(23, val)), 0)
    ...
    # Must be same local day (no cross-midnight sessions)
    if start_local.date() != end_local.date():
        _dbg("is_time_slot_available: ❌ crosses local midnight (single-day sessions only)")
        return False

    # **NEW**: reject past/started slots (compare in local tz)
    now_local = datetime.now(tz_local)
    if start_local < now_local:
        _dbg(f"is_time_slot_available: ❌ start in past vs now_local={now_local.isoformat()}")
        return False

    # Working day check
    if start_local.weekday() not in WORKING_DAYS:
        _dbg(f"is_time_slot_available: ❌ {start_local.strftime('%Y-%m-%d')} not in WORKING_DAYS {sorted(WORKING_DAYS)}")
        return False

    # Inside hours check (entire slot must fit)
    ws, we = dtime(WSTART, 0), dtime(WEND, 0)
    st, et = start_local.time(), end_local.time()
    if not (ws <= st and et <= we and st < et):
        _dbg(f"is_time_slot_available: ❌ outside working hours ({ws.strftime('%H:%M')}–{we.strftime('%H:%M')} local)")
        return False

    # Lunch overlap check (if defined)
    if LUNCH_START and LUNCH_END:
        if (st < LUNCH_END) and (et > LUNCH_START):
            _dbg(f"is_time_slot_available: ❌ overlaps lunch ({LUNCH_START.strftime('%H:%M')}–{LUNCH_END.strftime('%H:%M')} local)")
            return False

    # ---- Schedule valid → check Google Calendar for BUSY/FREE ----
    try:
        service = build("calendar", "v3", credentials=creds)
        events_result = service.events().list(
            calendarId=calendar_id,
            timeMin=in_start_dt.isoformat(),
...
def get_next_available_slots(
    ...
    *,
    ...
    if duration_minutes is None:
        try:
            duration_minutes = int(
                globals().get("APPOINTMENT_DURATION_MINUTES",
    ...
    def _inside_hours(dt_local):
        t = dt_local.time()
        ok = (dtime(WSTART, 0) <= t) and ((dt_local + timedelta(minutes=duration_minutes)).time() <= dtime(WEND, 0))
        if not ok:
            _dbg(f"get_next_available_slots: ⛔ outside working hours → {dt_local.strftime('%Y-%m-%d %H:%M')}")
        return ok
    ...
    def _in_lunch(dt_local):
        if LUNCH_START is None or LUNCH_END is None:
            return False
        st = dt_local.time()
        et = (dt_local + timedelta(minutes=duration_minutes)).time()
        hit = (st < LUNCH_END and et > LUNCH_START)
        if hit:
            _dbg(f"get_next_available_slots: 🍽️ overlaps lunch → {dt_local.strftime('%Y-%m-%d %H:%M')}")
        return hit
    ...
    # ---- seed start (NOW-aware) ----
    try:
        s = (from_start_iso or "").strip()
        req_utc = isoparse(s)
        if req_utc.tzinfo is None:
            req_utc = req_utc.replace(tzinfo=_pytz.UTC)
        else:
    ...
    # Snap into today’s first acceptable window
    if cur_local.weekday() not in WORKING_DAYS:
        _dbg(f"get_next_available_slots: 📅 non-working day {cur_local.weekday()} → jump to next working day")
        # jump to tomorrow 00:00 then set to first window hour
        d = cur_local
        while True:
        ...
        # move to the first window start if we’re before it; or next day if we’re past it
        ...
        if windows_today:
            ...
            if cur_local.time() < dtime(first_ws, 0):
                ...
                # jump to next working day’s first window
                d = cur_local
                while True:
                    d = (d + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
                    if d.weekday() in WORKING_DAYS:
    ...
    # ---- main scan (clinic schedule before Google) ----
    while cur_local < end_of_search and len(results) < limit:
        ...
        day = cur_local.date()
        # build localized windows for the day
        windows = []
        ...
        for wstart, wend in windows:
            ...
            while cur_local + timedelta(minutes=duration_minutes) <= wend and len(results) < limit:
                ...
                # Only now ask Google — clinic schedule already satisfied
                try:
