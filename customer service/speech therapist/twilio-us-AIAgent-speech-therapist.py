# update  10/15/25 time_saved  PERFECT
#  
# =========================
...
def insert_customer(
    ...
    if key in data:
        ...
        debug_print(f"insert_customer: 🟡 Updated existing record for {key} (status={customer_status})")
        return False
...
def voice():
    ...
    if stage == "intro":
        ...
        if choice:
            ...
            if choice == "1":
                # ✅ Booking
                debug_print("intent:📅 DTMF=1 → booking")
                session_data.setdefault(call_sid, {})
            ...
            if choice == "2":
                # ✅ Cancellation
                debug_print("intent:❌ DTMF=2 → cancel flow")
                session_data[call_sid] = {
            ...
            if choice == "4":
                # ✅ Update CC
                debug_print("intent:💳 DTMF=4 → update CC flow")
                session_data.setdefault(call_sid, {})
                session_data[call_sid].update({
                    "stage": "update_cc",
            ...
            if choice == "5":
                # ✅ Voicemail
                debug_print("intent:📩 DTMF=5 → voicemail")
                session_data.setdefault(call_sid, {})
        ...
        if not lower or lower in junk_inputs:
            debug_print(f"intent:⛔ Ignored junk input: '{lower}' — re-prompting without response")
            gather = make_gather(
                "Thank you for calling EPIC therapist. "
                "Say 'book appointment' or press 1. "
                "Say 'cancel appointment' or press 2. "
        ...
        # ------------------------------------------------------------------
        # 🎙️ Path 2: Speech-based matching (Foreign + English)
        # ------------------------------------------------------------------
        ...
        #   2️⃣ Customer FOUND but customer_status == "new" →
        #         → tell caller to contact clinic to verify information (end call)
        #   3️⃣ Customer FOUND and customer_status == "current" →
        #         → continue to "ask_time_date"
        #
        ...
        if not dob_date and speech_text:
            try:
                ...
                sd["stage"] = "collect_dob"
                g = make_gather(PROMPT_REPEAT_FULL, input="speech dtmf", timeout=3,
        ...
        if customer_status == "new":
            # Found but status=new → cannot proceed
            debug_print("collect_dob: 🟡 found record but status=new → require clinic verification")
            msg = (
                "We found your record, but it’s not yet verified. "
                "Please contact the clinic to confirm your information before booking an appointment. Goodbye!"
            )
        ...
        # ✅ Found + current → proceed to ask_time_date
        sd["stage"] = "ask_time_date"
        ...
        resp.append(g)
        resp.redirect("/voice")
        debug_print("collect_dob: ✅ existing verified customer → proceed to ask_time_date")
        return str(resp)
    ...
    # ----------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 💳 Simple collect_cc with DEBUG
        #   - Only captures credit card number (PAN)
        #   - Normalizes any Unicode digits via _uni
        #   - Accepts number even if Luhn fails
        #   - Logs detailed debug info for every step
        # ------------------------------------------------------------------

        def _digits_only(s: str) -> str:
            """Normalize Unicode digits (Arabic, Persian, etc.) → ASCII."""
            debug_print(f"[_digits_only] raw input: {repr(s)}")
            if not s:
                return ""
            out = []
            for ch in str(s):
                ...
                if "0" <= ch <= "9":
                    out.append(ch)
            result = "".join(out)
            debug_print(f"[_digits_only] normalized digits: {result}")
            return result
        ...
        def _luhn_ok(pan: str) -> bool:
            """Perform Luhn validation; log intermediate results."""
            digits = _digits_only(pan or "")
            debug_print(f"[_luhn_ok] checking digits: {digits}")
            if not (13 <= len(digits) <= 19):
                debug_print(f"[_luhn_ok] ❌ invalid length: {len(digits)}")
                return False
            total = 0
            double = False
            for ch in digits[::-1]:
                ...
                double = not double
            valid = (total % 10) == 0
            debug_print(f"[_luhn_ok] total={total}, valid={valid}")
            return valid
        ...
        raw_dtmf = (request.values.get("Digits") or "").strip()
        raw_speech = (speech_result or "").strip()
        debug_print(f"[collect_cc] stage start → DTMF='{raw_dtmf}' | speech='{raw_speech}'")

        # --- Silence handling ---
        if not raw_dtmf and not raw_speech:
            tries = session_data[call_sid].get("silence_cc", 0) + 1
            session_data[call_sid]["silence_cc"] = tries
            debug_print(f"[collect_cc] 🤐 silence detected, tries={tries}/3")

            if tries >= 3:
                debug_print("[collect_cc] too many silences → hangup")
                resp.say(gpt_speak("I’m still not hearing anything. Please call again later."), VOICE)
                resp.hangup()
                session_data.pop(call_sid, None)
                return str(resp)
        ...
        raw_pan = raw_dtmf or raw_speech
        digits = _digits_only(raw_pan)
        debug_print(f"[collect_cc] normalized digits: {digits} (len={len(digits)})")

        if not (13 <= len(digits) <= 19):
            debug_print(f"[collect_cc] ❌ Invalid length {len(digits)} — re-prompting.")
            gather = make_gather(
        ...
        if _luhn_ok(digits):
            debug_print(f"[collect_cc] ✅ Luhn check passed for {_mask_pan(digits)}")
        else:
            debug_print(f"[collect_cc] ⚠️ Luhn check failed for {_mask_pan(digits)} — accepted anyway.")

        # --- Save and advance ---
        customer["cc_number"] = digits
        session_data[call_sid]["stage"] = "collect_cc_exp"  # next step (change if needed)
        debug_print(f"[collect_cc] 🏁 saved cc_number={_mask_pan(digits)} → next stage=collect_cc_exp")
