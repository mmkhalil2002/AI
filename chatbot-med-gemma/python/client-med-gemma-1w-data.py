...
INPUT AND SELECTION
-------------------
Type a question in the left text box. The right panel displays images from the
newest date-MMDDYY dataset inside 'data' or beside this script. Click thumbnails to select
...
DEFAULT_MODEL = os.environ.get(
    "MEDGEMMA_MODEL", settings_from_file.get("MEDGEMMA_MODEL", "medgemma1.5:latest")
)
DEFAULT_MODELS = (DEFAULT_MODEL,)
# IMAGE_DIR can be absolute or relative to the directory containing this file.
SCRIPT_DIR = Path(__file__).resolve().parent
image_dir_setting = os.environ.get(
    "IMAGE_DIR", settings_from_file.get("IMAGE_DIR", "data")
)
...
def latest_dataset_folder():
    """Return the newest single image dataset using the two-window mechanism.

    The function searches inside IMAGE_DIR and beside this script. Valid
    date-MMDDYY folders are sorted chronologically. Unlike the two-window
    client, which uses dated[-2:], this one-window client uses dated[-1:] so
    exactly the newest dated dataset is selected.
    """
    candidates = []
    seen = set()
    ...
    for parent in (IMAGE_DIR, SCRIPT_DIR):
        ...
        for folder in parent.iterdir():
            if not folder.is_dir() or folder.name.startswith(".") or folder.name == "__pycache__":
                continue
            # When scanning beside the script, do not treat the data container
            # itself as a dataset; its child folders are checked separately.
            ...
            try:
                date = (
                    datetime.strptime(folder.name[5:], "%m%d%y")
                    if folder.name.lower().startswith("date-") else None
                )
                # Require all six MMDDYY digits because strptime can accept
                # some shortened values.
    ...
    ordinary = sorted(
        (item for item in candidates if item[0] is None),
        key=lambda item: item[1].name.lower(),
    )

    # Select one item only: the newest valid date folder. If no dated folder
    # exists, use the first ordinary folder that contains supported images.
    chosen = dated[-1:] if dated else ordinary[:1]
    return chosen[0][1] if chosen else None


def load_images():
    """Display images from the newest single dataset in three columns."""
    ...
    dataset_folder = latest_dataset_folder()
    if dataset_folder is None:
        ttk.Label(
            image_grid,
            text=(
                "No image dataset found. Create a folder such as "
                f"date-092126 inside:\n{IMAGE_DIR}"
            ),
            wraplength=230,
    ...
    # Search recursively inside the selected dataset, so its images may be
    # arranged in additional subfolders.
    paths = sorted(
        (
            p for p in dataset_folder.rglob("*")
            if p.is_file() and p.suffix.lower() in IMAGE_EXTENSIONS
        ),
        key=lambda p: str(p.relative_to(dataset_folder)).lower(),
    )
    if not paths:
        ttk.Label(image_grid, text=f"No images found in:\n{dataset_folder}", wraplength=230).grid(
    ...
    image_count.config(
        text=(
            f"{dataset_folder.name}: {len(thumbnails)} images "
            f"({THUMBNAIL_SIZE[0]} x {THUMBNAIL_SIZE[1]} px)"
...
window = tk.Tk()
window.title("MedGemma Client - newest single dataset v7")
