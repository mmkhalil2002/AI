...
USER_AGENT = "MedGemmaEducationalEKGDownloader/1.1 (educational Python client)"
SUPPORTED_FORMATS = {"JPEG", "PNG"}
DOWNLOAD_PAUSE_SECONDS = 3
RETRY_WAIT_SECONDS = (5, 15, 30, 60)
...
def request_bytes(request: urllib.request.Request, timeout: int) -> bytes:
    """Download bytes, retrying automatically when Wikimedia returns 429."""
    for attempt in range(len(RETRY_WAIT_SECONDS) + 1):
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                return response.read()
        except HTTPError as error:
            if error.code != 429 or attempt >= len(RETRY_WAIT_SECONDS):
                raise
            ...
            try:
                ...
                server_wait = 0
            wait_seconds = max(RETRY_WAIT_SECONDS[attempt], server_wait)
            print(
                f"Wikimedia is temporarily limiting downloads. "
...
def api_request(parameters: dict[str, str]) -> dict:
    ...
    return json.loads(request_bytes(request, timeout=60).decode("utf-8"))
...
def main() -> None:
    ...
    # Request more candidates than needed so a damaged or unsupported file
    # does not prevent the script from reaching the requested image count.
    titles = category_file_titles(min(count * 3, 50))
    if not titles:
    ...
    for title in titles:
        ...
        try:
            ...
            # Wikimedia asks automated clients to avoid rapid repeated calls.
