...
MODEL_PATH = "../../../"
MODEL_FILENAME = "cifar10-cnn-64-32-1024-L6604-A9820"
...
#   • conv2.weight, conv2.bias      (32-channel mid-level filters)   ✅ UPDATED
#   • conv3.weight, conv3.bias      (1024-channel higher-level filters) ✅ FIXED
#
...
#
# After conv2:                [32    x H/2 x W/2]     ✅ UPDATED
#   (NO pooling here to preserve spatial detail)
#
# After conv3:                [1024  x H/2 x W/2]     ✅ FIXED
#
# After GAP:                  [1024  x 1   x 1]       ✅ FIXED
# After flatten:              [1024]                  ✅ FIXED
...
# --------------------------
# If you EVER enable _init_conv2_static(), it MUST fill EXACTLY 32 output kernels
# because conv2.out_channels = 32 in this updated architecture.
# ============================================================
...
class StaticInitLearnableCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        ...
        # ------------------------------------------------------
        # ✅ LAYER 2: 64 → 32 channels ✅ UPDATED
        # ------------------------------------------------------
        # 64 input feature maps → 32 output feature maps
        # using 3×3 filters, padding=1 keeps spatial size.
        #
        # After the single pooling:
        #   input to conv2 : [B, 64, H/2, W/2]
        #   output of conv2: [B, 32, H/2, W/2]          ✅ UPDATED
        # ------------------------------------------------------
        self.conv2 = nn.Conv2d(
            in_channels=64,           # ✅ FIXED
            out_channels=32,          # ✅ UPDATED (was 128)
            kernel_size=3,
            padding=1,
            bias=True
        )
        ...
        # ------------------------------------------------------
        # ✅ BatchNorm for conv2 (normalizes 32 channels) ✅ UPDATED
        # ------------------------------------------------------
        # Why BatchNorm2d(32)?
        # --------------------
        ...
        # ------------------------------------------------------
        self.bn2 = nn.BatchNorm2d(32)   # ✅ UPDATED (was 128)

        # ------------------------------------------------------
        # ✅ LAYER 3: 32 → 1024 channels ✅ UPDATED
        # ------------------------------------------------------
        ...
        #
        #   input to conv3 : [B, 32, H/2, W/2]       ✅ UPDATED
        #   output of conv3: [B, 1024, H/2, W/2]     ✅ FIXED
        # ------------------------------------------------------
        self.conv3 = nn.Conv2d(
            in_channels=32,           # ✅ UPDATED (was 128)
        ...
        # ------------------------------------------------------
        # ✅ BatchNorm for conv3 (normalizes 1024 channels) ✅ FIXED
        # ------------------------------------------------------
        ...
        #   [B, 1024, 1, 1] → flatten → [B, 1024]     ✅ FIXED
        #
        ...
        # --------------------------------------
        # You requested: conv1=64, conv2=32, conv3=1024.
        #
        # • If you ever enable _init_conv2_static(), it MUST fill EXACTLY 32 kernels
        #   (because conv2 now has out_channels=32).
        #
        # Otherwise you'll get a shape mismatch or partially random channels.
        # ------------------------------------------------------
        if STATIC_FILTERS:
            self._init_conv1_static()
            # self._init_conv2_static()  # Only enable if you built EXACTLY 32 static kernels
            # self._init_conv3_static()  # Only enable if you built EXACTLY conv3 static init
    ...
    def _init_conv1_static(self):
        with torch.no_grad():                                              # disable gradients during manual init
            ...
            # Your conv1 is now:
            #   in_channels  = 3   (RGB)
            #   out_channels = 64  (64 feature maps / 64 filters)
            #   kernel       = 3x3
            #
            # So conv1 produces:
            #   [B, 3, H, W] → [B, 64, H, W]
            #
            # This gives the network higher early capacity than 16 channels.
            ...
            # ------------------------------------------------------------------
            # ASSIGN STATIC KERNELS → conv1 WEIGHTS (UPDATED FOR 64 CHANNELS)
            #
            # ✅ Your conv1 now has 64 output channels:
            #   out_channels = 64
            #
            # We currently have:
            ...
            #   • The remaining 19 filters repeat from the beginning (wrap-around)
            #
            # This guarantees:
            #   • conv1 starts with strong edge/corner/curve/line/gradient detectors
            #   • training converges easier (better inductive bias)
            #   • extra channels still exist (via repetition) so the network has 64 feature maps
            # ------------------------------------------------------------------

            for i in range(out_channels):                                  # loop over each output filter (0..63)
                k2d = kernels[i % num_kernels].to(device=w.device, dtype=w.dtype)  # pick kernel, move+cast
                for c in range(in_channels):                               # copy same kernel into each RGB channel
                    w[i, c].copy_(k2d)                                     # write into conv1 weight tensor

            repeats = max(0, out_channels - num_kernels)
            print(
    ...
    # ----------------------------------------------------------
    # STATIC INITIALIZATION FOR LAYER 2 (UPDATED FOR 32×64)
    # ----------------------------------------------------------
    def _init_conv2_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # into 32 structured, more semantic responses (textures / corners / parts).
            assert kh == 3 and kw == 3                                                  # ensure 3x3 kernel size
            assert in_channels == 64 and out_channels == 32                              # ensure expected channel sizes ✅ UPDATED
            ...
            #   • conv2 now combines those 64 maps into 32 stronger mid-level features:
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv2 WEIGHTS (UPDATED FOR 32×64)
            #
            # conv2 has:
            #   out_channels = 32   (filters)                               ✅ UPDATED
            ...
            #   • conv2 has 32×64 = 2048 small 3×3 kernels
    ...
    def _init_conv3_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # -----------------------------
            # Your conv3 is now:
            #   in_channels  = 32      (from conv2 out_channels) ✅ FIXED
            #   out_channels = 1024    (1024 feature maps / filters) ✅ FIXED
            #   kernel       = 3x3
            #
            # So conv3 produces:
            #   [B, 32, H/2, W/2] → [B, 1024, H/2, W/2] ✅ FIXED
            #
            ...
            assert in_channels == 32 and out_channels == 1024                            # ✅ FIXED (was 128 before)

            # ---------------------------------------------------------------------
            # FILTER DEFINITIONS (EACH 3×3, WRITTEN IN THREE ROWS)
            #
            # conv3 receives 32 feature maps (NOT raw RGB anymore). ✅ FIXED
            #
            # Meaning:
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv3 WEIGHTS (UPDATED FOR 1024×32) ✅ FIXED
            #
            # conv3 has:
            #   out_channels = 1024  (filters / output features) ✅ FIXED
            #   in_channels  = 32    (input features from conv2) ✅ FIXED
            #
            # Strategy:
            # ---------
            # We repeat a small bank of useful kernels across the 1024×32 connections.
            #
            # Why this can help:
            ...
            for out_idx in range(out_channels):                                       # loop over all 1024 output filters ✅ FIXED
                for in_idx in range(in_channels):                                     # loop over all 32 input feature maps ✅ FIXED
                    ...
                    k = kernels[(out_idx * 7 + in_idx * 3) % num_kernels].to(
                        device=w.device, dtype=w.dtype
                    )                                                                # ✅ FIXED: move+cast to match conv3 weights
    ...
    # ✔ Deeper feature extraction (up to 1024 channels) ✅ FIXED
    ...
    def forward(self, x):
        ...
        # Conv1: 3 → 64 channels, preserves H, W   ✅ FIXED
        #   [B, 3, H, W] → [B, 64, H, W]
        x = self.conv1(x)

        # BatchNorm on 64 channels (stabilizes activations) ✅ FIXED
        x = self.bn1(x)
        ...
        # Conv3: 32 → 1024 channels ✅ UPDATED
        #   [B, 32, H/2, W/2] → [B, 1024, H/2, W/2]
        x = self.conv3(x)
...
#   ----------------------------------------
#   In your current model conv2 is now 64 → 32 (NOT 64 → 128 anymore).
#   This means conv2 is a "compression + combination" layer:
#       • It merges 64 low-level maps into 32 mid-level maps.
#       • This can reduce compute and sometimes improve generalization.
#       • It also means conv3 receives fewer channels (32) and then expands strongly.
#
...
#   ✅ IMPORTANT (YOUR UPDATED CHANNEL PLAN):
#   ----------------------------------------
#   conv3 is now 32 → 1024 (NOT 128 → 1024).
#   This is a VERY strong expansion layer:
#       • conv2 compresses, conv3 expands massively.
#       • conv3 learns many high-level templates from a smaller mid-level basis.
#
...
#   • conv2: 64  → 32            ✅ UPDATED
#   • conv3: 32  → 1024          ✅ UPDATED
#   • GAP:  [B, 1024, H/2, W/2] → [B, 1024, 1, 1]
#   • FC:   1024 → num_classes
#
