...
# ✅ Also converts images to RGB (CIFAR-10 is already RGB, but kept for safety).
# ============================================================
...
EXPORT_ROOT = "cifar10"                 # <- you asked: directory called "cifar10"
...
def to_rgb_pil(img):
    ...
    CIFAR-10 images are already RGB PIL images, but we keep this consistent with your other exporters.
    """
...
def main():
    ...
    # ------------------------------------------------------------
    train_ds = datasets.CIFAR10(root=cache_root, train=True, download=True)
    test_ds  = datasets.CIFAR10(root=cache_root, train=False, download=True)

    # Class names (CIFAR-10 built-in)
    ...
    # Export images
    export_split("train", train_ds, train_root, class_names, train_counts, first_sizes, prefix="cifar10")
    export_split("test",  test_ds,  test_root,  class_names, test_counts,  first_sizes, prefix="cifar10")
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("✅ EXPORTED: CIFAR-10")
    for c, v in stats.items():
        print(
            f"{c:12s} | train={v['train']:6d} | test={v['test']:6d} | total={v['total']:6d} | first_WxH={v['first_wh']}"
        )
    print("=" * 80)
