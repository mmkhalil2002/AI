...
# These include:
#
#   • conv1.weight, conv1.bias      (64-channel low-level filters)
#   • conv2.weight, conv2.bias      (32-channel mid-level filters)
#   • conv3.weight, conv3.bias      (16-channel higher-level filters)
#
...
#
# After conv1:                [64  x H   x W]
# After pool1:                [64  x H/2 x W/2]
#
# After conv2:                [32  x H/2 x W/2]
# After pool2:                [32  x H/4 x W/4]
#
# After conv3:                [16  x H/4 x W/4]
#
# After GAP:                  [16  x 1   x 1]
# After flatten:              [16]
...
class StaticInitLearnableCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        ...
        # ------------------------------------------------------
        # ✅ LAYER 3: 32 → 16 channels ✅ UPDATED
        # ------------------------------------------------------
        ...
        # ------------------------------------------------------
        # ✅ BatchNorm for conv3 (normalizes 16 channels) ✅ UPDATED
        # ------------------------------------------------------
        # Why BatchNorm2d(16)?
        # --------------------
        # • Conv3 outputs 16 feature maps
        ...
        #   [B, C, H, W] → [B, C, 1, 1]
        #
        # In THIS model after conv3, C = 16:
        #   [B, 16, H', W'] → [B, 16, 1, 1]
        #
        ...
        # ✅ IMPORTANT UPDATE:
        # -------------------
        # Because conv3 now outputs 16 channels, GAP now outputs:
        #   [B, 16, 1, 1] → flatten → [B, 16]
        #
        ...
        # static filter bank actually fills/uses all 64 output maps.
        # ------------------------------------------------------
        if STATIC_FILTERS:
    ...
    def _init_conv1_static(self):
        with torch.no_grad():                                              # disable gradients during manual init
            ...
            # Your conv1 is now:
            #   in_channels  = 3    (RGB)
            #   out_channels = 64   (64 feature maps / 64 filters) ✅ UPDATED
            #   kernel       = 3x3
            #
            # So conv1 produces:
            #   [B, 3, H, W] → [B, 64, H, W] ✅ UPDATED
            #
            # NOTE:
            # • This is a good balance between capacity and speed.
            # • Static init still works the same way; we just fill 64 filters instead of 128.
            ...
            #   • The remaining 19 filters repeat from the beginning (wrap-around) ✅ UPDATED
            #
            # This guarantees:
            ...
            for i in range(out_channels):                                  # loop over each output filter (0..63) ✅ UPDATED
    ...
    # ----------------------------------------------------------
    # STATIC INITIALIZATION FOR LAYER 2 (UPDATED FOR 64×32) ✅ UPDATED
    # ----------------------------------------------------------
    def _init_conv2_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # --------------------------------------------
            # Your conv2 is now:
            #   in_channels  = 64    (from conv1 out_channels) ✅ UPDATED
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv2 WEIGHTS (UPDATED FOR 32×64) ✅ UPDATED
            #
            # conv2 has:
            #   out_channels = 32   (filters) ✅ UPDATED
            #   in_channels  = 64   (input feature maps from conv1) ✅ UPDATED
            #
            ...
            #   • conv2 has 32×64 = 2048 small 3×3 kernels ✅ UPDATED
    ...
    # ----------------------------------------------------------
    # STATIC INITIALIZATION FOR LAYER 3 (UPDATED FOR 64 FEATURES) ✅ UPDATED
    # ----------------------------------------------------------
    def _init_conv3_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            #
            # So conv3 produces:
            #   [B, 32, H/4, W/4] → [B, 64, H/4, W/4] ✅ UPDATED
            #
            ...
            assert in_channels == 32 and out_channels == 64                             # expect exact conv3 shape ✅ UPDATED
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv3 WEIGHTS (UPDATED FOR 64×32) ✅ UPDATED
            #
            # conv3 has:
            #   out_channels = 64  (filters / output features) ✅ UPDATED
            #   in_channels  = 32  (input features from conv2) ✅ UPDATED
            #
            # Strategy:
            # ---------
            # We repeat a small bank of useful kernels across the 64×32 connections.
            #
            # Why this can help:
    ...
    # ✔ Feature hierarchy (16 → 32 → 64 channels) ✅ UPDATED
    ...
    def forward(self, x):
        ...
        # Conv1: 3 → 16 channels, preserves H, W ✅ UPDATED
        #   [B, 3, H, W] → [B, 16, H, W]
        x = self.conv1(x)

        # BatchNorm on 16 channels (stabilizes activations) ✅ UPDATED
        x = self.bn1(x)
        ...
        # Converts:
        #   [B, 64, H/4, W/4] → [B, 64, 1, 1] ✅ UPDATED
        x = self.gap(x)

        # Flatten channel dimension only
        #   [B, 64, 1, 1] → [B, 64] ✅ UPDATED
        x = torch.flatten(x, 1)
