...
# ============================================================
# The client can connect to any classifier server.
#
# Each server has its own unique port so all servers can run
...
#   ALL_PORT=6080
#
# If the user does not select a server from the menu, the client
# uses the MED server by default.
# ============================================================

MED_PORT = get_int("MED_PORT", 5055)
RME_PORT = get_int("RME_PORT", 6060)
CIFAR10_PORT = get_int("CIFAR10_PORT", 6070)
ALL_PORT = get_int("ALL_PORT", 6080)

DEFAULT_SERVER_NAME = "MED"
DEFAULT_SERVER_PORT = MED_PORT

# Compatibility alias. This value is updated after the user selects
# a server from the port menu.
TCP_PORT = DEFAULT_SERVER_PORT
...
def choose_server_port() -> Tuple[str, int]:
    """
    Ask the user which classifier server to use.

    If the user presses ENTER without selecting anything, the MED
    server is selected by default.

    Available servers:
      1) MED      -> MED_PORT
      2) RME      -> RME_PORT
      3) CIFAR10  -> CIFAR10_PORT
      4) ALL      -> ALL_PORT
      5) Custom   -> user manually enters a port number
    """
    print()
    print("============================================================")
    print("SELECT CLASSIFIER SERVER PORT")
    print("============================================================")
    print(f"1 - MED server      MED_PORT={MED_PORT}       [DEFAULT]")
    print(f"2 - RME server      RME_PORT={RME_PORT}")
    print(f"3 - CIFAR10 server  CIFAR10_PORT={CIFAR10_PORT}")
    print(f"4 - ALL server      ALL_PORT={ALL_PORT}")
    print("5 - Custom port")
    print("============================================================")
    print("Press ENTER to use MED server.")
    choice = input("Select server option: ").strip().lower()

    if choice == "":
        return "MED", int(MED_PORT)

    if choice in ("1", "med", "m"):
        return "MED", int(MED_PORT)

    if choice in ("2", "rme", "r"):
        return "RME", int(RME_PORT)

    if choice in ("3", "cifar", "cifar10", "c"):
        return "CIFAR10", int(CIFAR10_PORT)

    if choice in ("4", "all", "a"):
        return "ALL", int(ALL_PORT)

    if choice in ("5", "custom", "p", "port"):
        custom_port = input("Enter custom TCP port: ").strip()
        if not custom_port:
            print(f"No custom port entered. Using MED_PORT={MED_PORT}.")
            return "MED", int(MED_PORT)
        ...
        try:
            ...
            print(f"Invalid custom port. Using MED_PORT={MED_PORT}.")
            return "MED", int(MED_PORT)

    print(f"Invalid selection. Using MED_PORT={MED_PORT}.")
    return "MED", int(MED_PORT)
...
def print_result_to_screen(image_path: str, result: Dict[str, Any]):
    ...
    conf_pct = result.get("final_confidence_percent", result.get("confidence_percent", None))
    if conf_pct is not None:
        print(f"CONFIDENCE       : {float(conf_pct):.2f}%")
    else:
...
def main():
    ...
    Main program:
      1) Read .env settings
      2) Show server port menu
      3) Ask for server IP
      4) Read images from TEST_IMAGE_DIR
      5) Send each image as raw RGB using 1-byte width/height/channels
      6) Print server classification result
    """
    print("============================================================")
    ...
    print(f"MED_PORT                  : {MED_PORT}")
    print(f"RME_PORT                  : {RME_PORT}")
    print(f"CIFAR10_PORT              : {CIFAR10_PORT}")
    print(f"ALL_PORT                  : {ALL_PORT}")
    ...
    selected_server_name, selected_server_port = choose_server_port()

    # Update TCP_PORT after menu selection so all later code uses
    # the selected server port.
    global TCP_PORT
    TCP_PORT = selected_server_port

    print()
    print(f"[CLIENT] Selected server : {selected_server_name}")
    print(f"[CLIENT] Selected port   : {TCP_PORT}")
