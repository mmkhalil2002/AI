...
class StaticInitLearnableCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        ...
        #
        # NOTE:
        #   This model is now IMAGE-SIZE INDEPENDENT because we
...
def train_model(model, train_loader, device, num_epochs=2, lr=3e-3):
   ...
   # Create the optimizer that is responsible for *training the neural network*.
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=lr,                    # ✅ Base LR aligned with OneCycle max_lr
        weight_decay=1e-4
    )
    ...
    max_lr=lr,                      # ✅ PEAK learning rate (the highest LR reached during the cycle).
                                    #    IMPORTANT:
                                    #      • This is NOT the starting LR.
                                    #      • OneCycleLR will start LOWER than max_lr (see div_factor),
                                    #        then ramp UP to max_lr, then decay DOWN again.
                                    #
                                    # Why max_lr matters:
                                    #   • Higher max_lr helps explore the loss surface early.
                                    #   • Too high → training becomes unstable / loss spikes / NaNs.
                                    #   • Typical for CIFAR-10 CNN + AdamW: 1e-3 to 5e-3.

    steps_per_epoch=len(train_loader),
                                    # ✅ Number of optimizer updates in ONE epoch.
                                    #    This MUST equal the number of mini-batches in your DataLoader.
                                    #
                                    # Example:
                                    #   • CIFAR-10 train size = 50,000
                                    #   • batch_size = 128
                                    #   • steps_per_epoch ≈ 50,000 / 128 ≈ 391 batches
                                    #
                                    # OneCycleLR needs this to know:
                                    #   “How many total LR steps will happen in one epoch?”

    epochs=num_epochs,              # ✅ Total number of epochs for the FULL LR schedule.
                                    #    OneCycleLR plans the entire LR curve across:
                                    #       total_steps = steps_per_epoch * epochs
                                    #
                                    # IMPORTANT:
                                    #   Because this scheduler is ITERATION-BASED:
                                    #     ✅ call scheduler.step() ONCE PER MINI-BATCH
                                    #     ❌ do NOT call scheduler.step() once per epoch

    pct_start=0.1,                  # ✅ Fraction of training used for the "warm-up" (LR INCREASE phase).
                                    #
                                    # With pct_start=0.1:
                                    #   • First 10% of total_steps:
                                    #       LR ramps UP from start_lr → max_lr
                                    #   • Remaining 90%:
                                    #       LR decays DOWN from max_lr → final_lr
                                    #
                                    # Why lowering pct_start can reduce early loss faster:
                                    #   • Shorter warmup = reach effective learning rate sooner
                                    #   • Helps loss drop earlier (but if too aggressive can destabilize)

    anneal_strategy="cos",          # ✅ Controls HOW the LR decays after reaching max_lr.
                                    #
                                    # "cos" = cosine annealing:
                                    #   • Smooth curve
                                    #   • Gentle decay near the end
                                    #   • Often gives better accuracy/generalization than linear
                                    #
                                    # Alternative:
                                    #   "linear" = straight-line decay (simpler, sometimes harsher)

    div_factor=5.0,                 # ✅ Controls the STARTING learning rate.
                                    #
                                    # OneCycleLR computes:
                                    #   start_lr = max_lr / div_factor
                                    #
                                    # Example:
                                    #   if max_lr = 0.003 and div_factor = 5:
                                    #       start_lr = 0.003 / 5 = 0.0006
                                    #
                                    # Why this affects "initial loss":
                                    #   • If start_lr is TOO SMALL → early learning is slow
                                    #   • If start_lr is TOO BIG   → early training can be unstable
                                    #
                                    # Your choice (5.0) is more aggressive than the default (25),
                                    # meaning the model starts learning faster at the beginning.

    final_div_factor=1e3            # ✅ Controls the FINAL learning rate at the end of the cycle.
                                    #
                                    # OneCycleLR computes:
                                    #   final_lr = start_lr / final_div_factor
                                    #
                                    # Example:
                                    #   start_lr = 0.0006
                                    #   final_div_factor = 1000
                                    #   final_lr = 0.0006 / 1000 = 6e-7
                                    #
                                    # Why this matters:
                                    #   • Very small final LR helps “fine-tune” weights near the end
                                    #   • Too small too early → training can stall
                                    #   • This parameter helps keep the end of training stable and smooth
)
...
def main():
    ...
    test_transform = transforms.Compose([
        ...
        # • Works well with ReLU-based CNNs.
        #
        # IMPORTANT NOTE:
        # ⚠️ Ideally, TRAIN and TEST normalization
        #    SHOULD MATCH.
        #
        # If training used ImageNet stats,
        # test SHOULD ALSO use ImageNet stats.
        #
        # Keeping this as-is because you requested comments only.
        #
        # SAFE FOR ANY IMAGE SIZE:
        # • Pixel-wise operation.
        # • No dependency on H or W.
        # --------------------------------------------------------
        transforms.Normalize(
            mean=[0.5, 0.5, 0.5],
            std=[0.5, 0.5, 0.5]
