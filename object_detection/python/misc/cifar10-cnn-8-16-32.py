...
MODEL_PATH = "../../../"
MODEL_FILENAME = "cifar10-cnn-8-16-32"
...
# These include:
#
#   • conv1.weight, conv1.bias      (8-channel low-level filters)   ✅ UPDATED
#   • conv2.weight, conv2.bias      (16-channel mid-level filters)  ✅ UPDATED
#   • conv3.weight, conv3.bias      (32-channel higher-level filters)✅ UPDATED
#
#   • bn1.weight, bn1.bias          (BatchNorm scale/shift for conv1: 8)   ✅ UPDATED
#   • bn2.weight, bn2.bias          (BatchNorm scale/shift for conv2: 16)  ✅ UPDATED
#   • bn3.weight, bn3.bias          (BatchNorm scale/shift for conv3: 32)  ✅ UPDATED
#
#   • fc.weight,  fc.bias           (final classifier: 32 → num_classes)   ✅ UPDATED
#
...
#
# After conv1:                [8   x H   x W]        ✅ UPDATED
# After pool1:                [8   x H/2 x W/2]      ✅ UPDATED
#
# After conv2:                [16  x H/2 x W/2]      ✅ UPDATED
# After pool2:                [16  x H/4 x W/4]      ✅ UPDATED
#
# After conv3:                [32  x H/4 x W/4]      ✅ UPDATED
#
# After GAP:                  [32  x 1   x 1]        ✅ UPDATED
# After flatten:              [32]                   ✅ UPDATED
...
class StaticInitLearnableCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        ...
        # ------------------------------------------------------
        # LAYER 1: 3 → 8 channels ✅ UPDATED
        # ------------------------------------------------------
        # 3 input channels (RGB) → 8 feature maps using 3x3 filters
        ...
        # ------------------------------------------------------
        # BatchNorm for conv1 (normalizes 8 output channels) ✅ UPDATED
        # ------------------------------------------------------
        self.bn1 = nn.BatchNorm2d(8)  # ✅ UPDATED

        # ------------------------------------------------------
        # LAYER 2: 8 → 16 channels ✅ UPDATED
        # ------------------------------------------------------
        # 8 input feature maps → 16 output feature maps
        # using 3×3 filters, padding=1 keeps spatial size.
        #
        # After first pooling:
        #   input to conv2 : [B,   8, H/2, W/2]
        #   output of conv2: [B,  16, H/2, W/2]
        # ------------------------------------------------------
        self.conv2 = nn.Conv2d(
            in_channels=8,     # ✅ UPDATED
        ...
        # ------------------------------------------------------
        # ✅ LAYER 3: 16 → 32 channels ✅ UPDATED
        # ------------------------------------------------------
        ...
        self.conv3 = nn.Conv2d(
            in_channels=16,    # ✅ (same as before, but now feeds 32)
        ...
        # ------------------------------------------------------
        self.bn3 = nn.BatchNorm2d(32)  # ✅ UPDATED

        # ------------------------------------------------------
        # 🔑 GLOBAL AVERAGE POOLING (IMAGE-SIZE INDEPENDENT)
        # ------------------------------------------------------
        # In THIS model after conv3, C = 32 ✅ UPDATED:
        #   [B, 32, H', W'] → [B, 32, 1, 1]
        # ------------------------------------------------------
        ...
        #   nn.Linear(32, num_classes) ✅ UPDATED
        # ------------------------------------------------------
        self.fc = nn.Linear(32, num_classes)  # ✅ UPDATED
        ...
        # static filter bank actually fills/uses all 8 output maps ✅ UPDATED
        # ------------------------------------------------------
        if STATIC_FILTERS:
...
def _init_conv1_static(self):
    with torch.no_grad():                                              # disable gradients during manual init
        ...
        # ✅ UPDATED FOR YOUR NEW MODEL (conv1=8, conv2=16, conv3=32):
        # ----------------------------------------------------------
        # Your conv1 is now:
        #   in_channels  = 3    (RGB)
        #   out_channels = 8    (8 feature maps / 8 filters) ✅ UPDATED
        #   kernel       = 3x3
        #
        # So conv1 produces:
        #   [B, 3, H, W] → [B, 8, H, W] ✅ UPDATED
        #
        # NOTE:
        # • This is still lightweight compared to huge models.
        # • Static init still works the same way; we just fill 8 filters now.
        ...
        # ------------------------------------------------------------------
        # ASSIGN STATIC KERNELS → conv1 WEIGHTS (UPDATED FOR 8 CHANNELS)
        #
        # ✅ Your conv1 now has 8 output channels:
        #   out_channels = 8 ✅ UPDATED
        #
        # We currently have:
        #   num_kernels = {num_kernels} handcrafted 3×3 kernels
        #
        # Because 8 < num_kernels:
        #   • We take ONLY the first 8 kernels (truncate) ✅ UPDATED
        ...
        #   • all 8 filters are UNIQUE handcrafted kernels
        # ------------------------------------------------------------------

        for i in range(out_channels):                                  # loop over each output filter (0..7) ✅ UPDATED
            k2d = kernels[i].to(w.dtype)                               # take first 8 kernels directly (no wrap) ✅ UPDATED
    ...
    # ----------------------------------------------------------
    # STATIC INITIALIZATION FOR LAYER 2 (UPDATED FOR 16×8) ✅ UPDATED
    # ----------------------------------------------------------
    def _init_conv2_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # ----------------------------------------------------------
            # Your conv2 is now:
            #   in_channels  = 8     (from conv1 out_channels) ✅ UPDATED
            #   out_channels = 16    (16 feature maps / filters) ✅ UPDATED
            #   kernel       = 3x3
            #
            # So conv2 produces:
            #   [B,  8, H/2, W/2] → [B, 16, H/2, W/2] ✅ UPDATED
            #
            ...
            #
            # conv2 receives 8 feature maps (from conv1). ✅ UPDATED
            #
            # Meaning:
            #   • conv1 produced primitive detectors (edges/corners/etc.) across 8 maps ✅ UPDATED
            #   • conv2 combines those 8 maps into 16 mid-level features:
            #       - parts, textures, repeated patterns
            #       - stronger combinations of the static conv1 responses
            #
            # NOTE:
            #   Here out_channels is larger than in_channels (16 > 8). ✅ UPDATED
            #   This is also valid — it is a deliberate "feature expansion" step:
            #     • increases representational capacity
            #     • allows multiple different mixtures from the same 8 conv1 maps
            #     • works well with BN + ReLU
            # ---------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv2 WEIGHTS (UPDATED FOR 16×8) ✅ UPDATED
            #
            # conv2 has:
            #   out_channels = 16   (filters) ✅ UPDATED
            #   in_channels  = 8    (input feature maps from conv1) ✅ UPDATED
            #
            # ✅ Why repetition is OK here:
            #   • conv2 has 16×8 = 128 small 3×3 kernels ✅ UPDATED
            ...
            for out_idx in range(out_channels):                                       # loop over all 16 output filters ✅ UPDATED
                for in_idx in range(in_channels):                                     # loop over all 8 input feature maps ✅ UPDATED
    ...
    def _init_conv3_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            #
            # So conv3 produces:
            #   [B, 16, H/4, W/4] → [B, 32, H/4, W/4] ✅ UPDATED
            #
            ...
            assert in_channels == 16 and out_channels == 32                             # expect exact conv3 shape ✅ UPDATED
            ...
            #
            # Meaning:
            #   • conv1: low-level primitives (edges/corners/lines) across 8 maps ✅ UPDATED
            #   • conv2: mid-level combinations (textures/parts) expanded to 16 maps ✅ UPDATED
            #   • conv3: stronger mid/high-level compositions (parts → object patterns) into 32 maps ✅ UPDATED
            #
            # IMPORTANT:
            # ----------
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv3 WEIGHTS (UPDATED FOR 32×16) ✅ UPDATED
            #
            # conv3 has:
            #   out_channels = 32  (filters / output_
    ...
    def forward(self, x):
        ...
        # Conv1: 3 → 8 channels, preserves H, W ✅ UPDATED
        #   [B, 3, H, W] → [B, 8, H, W]
        x = self.conv1(x)

        # BatchNorm on 8 channels (stabilizes activations) ✅ UPDATED
        x = self.bn1(x)
        ...
        # Conv2: 8 → 16 channels ✅ UPDATED
        #   [B, 8, H/2, W/2] → [B, 16, H/2, W/2]
        x = self.conv2(x)
        ...
        # Conv3: 16 → 32 channels ✅ UPDATED
        #   [B, 16, H/4, W/4] → [B, 32, H/4, W/4]
        x = self.conv3(x)
