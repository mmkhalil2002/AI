...
MODEL_PATH = "../../../"
MODEL_FILENAME = "cifar10-cnn-16-32-64"
...
# These include:
#
#   • conv1.weight, conv1.bias      (16-channel low-level filters)
#   • conv2.weight, conv2.bias      (32-channel mid-level filters)
#   • conv3.weight, conv3.bias      (64-channel higher-level filters)
#
...
#
# After conv1:                [16  x H   x W]
# After pool1:                [16  x H/2 x W/2]
#
# After conv2:                [32  x H/2 x W/2]
# After pool2:                [32  x H/4 x W/4]
#
# After conv3:                [64  x H/4 x W/4]
#
# After GAP:                  [64  x 1   x 1]
# After flatten:              [64]
...
class StaticInitLearnableCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        ...
        # ------------------------------------------------------
        # BatchNorm for conv1 (normalizes 16 output channels) ✅ UPDATED
        # ------------------------------------------------------
        self.bn1 = nn.BatchNorm2d(16)

        # ------------------------------------------------------
        # LAYER 2: 16 → 32 channels ✅ UPDATED
        # ------------------------------------------------------
        # 16 input feature maps → 32 output feature maps
        # using 3×3 filters, padding=1 keeps spatial size.
        #
        # After first pooling:
        #   input to conv2 : [B,  16, H/2, W/2]
        #   output of conv2: [B,  32, H/2, W/2]
        # ------------------------------------------------------
        ...
        # ------------------------------------------------------
        # ✅ LAYER 3: 32 → 64 channels ✅ UPDATED
        # ------------------------------------------------------
        # After second pooling:
        #   input to conv3 : [B,  32, H/4, W/4]
        #   output of conv3: [B,  64, H/4, W/4]
        # ------------------------------------------------------
        ...
        # ------------------------------------------------------
        # ✅ BatchNorm for conv3 (normalizes 64 channels) ✅ UPDATED
        # ------------------------------------------------------
        ...
        # ------------------------------------------------------
        # In THIS model after conv3, C = 64 ✅ UPDATED:
        #   [B, 64, H', W'] → [B, 64, 1, 1]
        # ------------------------------------------------------
        ...
        #   nn.Linear(64, num_classes) ✅ UPDATED
        # ------------------------------------------------------
        ...
        # static filter bank actually fills/uses all 16 output maps ✅ UPDATED
        # ------------------------------------------------------
        if STATIC_FILTERS:
    ...
    def _init_conv1_static(self):
        with torch.no_grad():                                              # disable gradients during manual init
            ...
            # ✅ UPDATED FOR YOUR NEW MODEL (16 → 32 → 64):
            # --------------------------------------------
            # Your conv1 is now:
            #   in_channels  = 3    (RGB)
            #   out_channels = 16   (16 feature maps / 16 filters) ✅ UPDATED
            #   kernel       = 3x3
            #
            # So conv1 produces:
            #   [B, 3, H, W] → [B, 16, H, W] ✅ UPDATED
            #
            # NOTE:
            # • This is faster and lighter than 64 filters.
            # • Static init still works the same way; we just fill 16 filters now.
            ...
            # ------------------------------------------------------------------
            # ASSIGN STATIC KERNELS → conv1 WEIGHTS (UPDATED FOR 16 CHANNELS)
            #
            # ✅ Your conv1 now has 16 output channels:
            ...
            #
            # Because 16 < 45:
            #   • We take ONLY the first 16 kernels (truncate) ✅ UPDATED
            ...
            #   • all 16 filters are UNIQUE handcrafted kernels
            # ------------------------------------------------------------------

            for i in range(out_channels):                                  # loop over each output filter (0..15) ✅ UPDATED
                k2d = kernels[i].to(w.dtype)                               # take first 16 kernels directly (no wrap)
    ...
    def _init_conv2_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # --------------------------------------------
            # Your conv2 is now:
            #   in_channels  = 16    (from conv1 out_channels) ✅ UPDATED
            #   out_channels = 32    (32 feature maps / filters) ✅ UPDATED
            #   kernel       = 3x3
            #
            # So conv2 produces:
            #   [B, 16, H/2, W/2] → [B, 32, H/2, W/2] ✅ UPDATED
            #
            ...
            #
            # conv2 receives 16 feature maps (NOT 64 anymore). ✅ UPDATED
            #
            # Meaning:
            #   • conv1 produced primitive detectors (edges/corners/etc.)
            #   • conv2 combines those 16 maps into 32 stronger mid-level features:
            #       - parts, textures, repeated patterns
            #       - richer combinations of the static conv1 responses
            #
            # NOTE:
            #   Here out_channels is larger than in_channels (32 > 16). ✅ UPDATED
            #   This is NOT wrong — it is a deliberate "feature expansion" step:
            #     • increases representation capacity
            #     • allows more diverse mid-level feature maps
            #     • often improves accuracy when paired with BN + ReLU
            # ---------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv2 WEIGHTS (UPDATED FOR 32×16) ✅ UPDATED
            #
            # conv2 has:
            #   out_channels = 32   (filters) ✅ UPDATED
            #   in_channels  = 16   (input feature maps from conv1) ✅ UPDATED
            #
            # ✅ Why repetition is OK here:
            #   • conv2 has 32×16 = 512 small 3×3 kernels ✅ UPDATED
    ...
    # ----------------------------------------------------------
    # STATIC INITIALIZATION FOR LAYER 3 (UPDATED FOR 16 FEATURES) ✅ UPDATED
    # ----------------------------------------------------------
    def _init_conv3_static(self):
        with torch.no_grad():                                                           # disable gradients (manual init)
            ...
            # ✅ UPDATED FOR YOUR NEW MODEL (64 → 32 → 16):
            # --------------------------------------------
            # Your conv3 is now:
            #   in_channels  = 32    (from conv2 out_channels) ✅ UPDATED
            #   out_channels = 16    (16 feature maps / filters) ✅ UPDATED
            #   kernel       = 3x3
            #
            # So conv3 produces:
            #   [B, 32, H/4, W/4] → [B, 16, H/4, W/4] ✅ UPDATED
            #
            ...
            assert in_channels == 32 and out_channels == 16                             # expect exact conv3 shape ✅ UPDATED

            # ---------------------------------------------------------------------
            # FILTER DEFINITIONS (EACH 3×3, WRITTEN IN THREE ROWS)
            #
            # conv3 receives 32 feature maps (NOT raw RGB anymore). ✅ UPDATED
            #
            # Meaning:
            ...
            # ---------------------------------------------------------------------
            # ASSIGN FILTERS TO ALL conv3 WEIGHTS (UPDATED FOR 16×32) ✅ UPDATED
            #
            # conv3 has:
            #   out_channels = 16  (filters / output features) ✅ UPDATED
            #   in_channels  = 32  (input features from conv2) ✅ UPDATED
            #
            # Strategy:
            # ---------
            # We repeat a small bank of useful kernels across the 16×32 connections.
            #
            # Why this can help:
   ...
   # ----------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # ✔ Two-stage pooling reduces compute while keeping structure
    # ✔ Feature hierarchy (64 → 32 → 16 channels)
    ...
    def forward(self, x):
        ...
        # Conv1: 3 → 64 channels, preserves H, W ✅ UPDATED
        #   [B, 3, H, W] → [B, 64, H, W]
        x = self.conv1(x)

        # BatchNorm on 64 channels (stabilizes activations) ✅ UPDATED
        x = self.bn1(x)
        ...
        #   [B, 32, H/2, W/2] → [B, 32, H/4, W/4] ✅ UPDATED
        x = self.pool(x)
        ...
        # Conv3: 32 → 16 channels ✅ UPDATED
        #   [B, 32, H/4, W/4] → [B, 16, H/4, W/4]
        x = self.conv3(x)
