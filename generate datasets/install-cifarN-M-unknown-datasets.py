...
#
#   The printed number is:
#       train_count + test_count
#
#   for selected classes and unknown only.
# ============================================================
...
def print_selected_plus_unknown_counts(class_names_selected, train_counts, test_counts):
    ...
    where number_of_images = train_count + test_count
    """
    print("\n" + "=" * 70)
    ...
    for cname_raw in class_names_selected:
        cname = safe_name(cname_raw)
        total_selected = int(train_counts.get(cname, 0)) + int(test_counts.get(cname, 0))
        print(f"{cname:20s} : {total_selected}")

    unknown_name = safe_name("unknown")
    unknown_total = int(train_counts.get(unknown_name, 0)) + int(test_counts.get(unknown_name, 0))
    print(f"{unknown_name:20s} : {unknown_total}")
